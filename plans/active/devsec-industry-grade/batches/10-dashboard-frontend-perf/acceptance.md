...
- **S-054 (tokens swept):** The high-frequency inline hex/rgba on shared primitives (`#fff`×32, brand-green `#2f6656`×5) are replaced with their existing `:root` tokens; the inline-value count outside `:root` drops materially from the 60 hex + 212 rgba baseline, with only genuine one-off rgba effects left inline. (Note — added by batch 09: the orphaned `components/OverviewView.tsx` cited here at `:45,49,65` for raw Tailwind `black/white` opacities was **deleted** by batch 09 (S-036), so that specific mixed-idiom drift is already resolved by deletion; S-054 now narrows to the live `index.css` token sweep. A broader Tailwind-`black/white`-opacity family still lives in non-case components like `RotationStatusCard.tsx`/`DependenciesView.tsx` — out of S-054's "shared primitives" scope, do not chase it here.)
...
| `grep -nE "#fff|#2f6656" dashboard-ui/src/index.css` before vs after | Proves S-054: the high-frequency inline hex on shared primitives is gone (now `var(--…)` tokens), with the count materially reduced. (Path `dashboard-ui/src/components/OverviewView.tsx` removed from this check — batch 09 deleted that orphan file.) |
