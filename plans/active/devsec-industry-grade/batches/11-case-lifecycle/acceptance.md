...
- [ ] A just-closed case stays visible for one cycle as an affirmative "Verified ✓ in scan X" state rather than silently dropping out of the attention list (`CaseCard.tsx:228-234` no longer relies solely on "not found in the latest scan" framing; the resolved card names the scan that closed it). `cd dashboard-ui && npm run build` is clean and `npm run lint` passes after the UI change.
