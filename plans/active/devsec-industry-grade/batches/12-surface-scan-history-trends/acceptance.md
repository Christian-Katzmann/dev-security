...
| Component test (`cd dashboard-ui && npm test`): the batch-07 vitest harness now exists (vitest + @testing-library/react + jest-axe; config in `vite.config.ts` `test` block + `src/test/setup.ts`). Add a spec that renders the history/trends panel and asserts it reads the history series; select base/head and assert the diff carries both scans. | Synthesis "Suggested validation" for S-039 — proves the panel fetches/renders and the base/head selection actually drives the diff. |
