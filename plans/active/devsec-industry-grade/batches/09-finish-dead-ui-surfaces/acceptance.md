...
- [ ] The orphaned trio `dashboard-ui/src/components/OverviewView.tsx`, `components/CasesView.tsx`, and `components/CaseCard.tsx` is removed (or any genuinely wanted part adopted into the live inline equivalents and re-skinned to Mistglass, with the orphaned files then deleted). The live act path remains the inline `OverviewView` (`App.tsx:1789`), inline `CasesView` (`App.tsx:2343`), and inline `CaseDetailCard` — one source of truth for the case control.
- [ ] No remaining import of the deleted files anywhere: `grep -rn "components/OverviewView\|components/CasesView\|components/CaseCard" dashboard-ui/src/` returns nothing (or only references to the live inline components, never the deleted module paths).
...
- [ ] The Activity event-feed filter chips at `App.tsx:2887` are no longer dead controls: **either** each chip carries `active`/`onClick` state and clicking it filters the event feed (mirroring the wired CasesView chips at `App.tsx:2440-2450`), **or** they are rendered as plain static labels with no chip affordance (no clickable-looking control that does nothing).
...
| `grep -rn "components/OverviewView\|components/CasesView\|components/CaseCard" dashboard-ui/src/` | Confirms no import of the deleted orphaned files remains (S-036 — matrix + synthesis "Suggested validation"). |
