    ...
    WKUIDelegate
{
    ...
    private let minZoomLevel: CGFloat = 0.7
    private let maxZoomLevel: CGFloat = 1.6
    ...
    private func installMainMenu() {
        ...
        let zoomInPlus = NSMenuItem(title: "Zoom In", action: #selector(zoomIn(_:)), keyEquivalent: "+")
        zoomInPlus.keyEquivalentModifierMask = [.command, .shift]
        zoomInPlus.isAlternate = true
        zoomInPlus.target = self
        viewMenu.addItem(zoomInPlus)

        let zoomOut = NSMenuItem(title: "Zoom Out", action: #selector(zoomOut(_:)), keyEquivalent: "-")
        zoomOut.target = self
        viewMenu.addItem(zoomOut)

        let actualSize = NSMenuItem(title: "Actual Size", action: #selector(resetZoom(_:)), keyEquivalent: "0")
    ...
    private func setZoom(_ nextZoom: CGFloat) {
        zoomLevel = min(max(nextZoom, minZoomLevel), maxZoomLevel)
        applyZoom()
    }
    ...
    private func killServer() {
        ...
        {
            sweepPort(bport)
            try? FileManager.default.removeItem(atPath: backendPortPath)
        }
    }
