...
- **`~/.security-observatory/`** — local SQLite database containing scan history, finding text (which can include short snippets of secret matches and source patterns), and repo metadata.
...
- **No multi-user model.** The single operator who runs `security-scan` is assumed to be the owner of the findings. There is no RBAC, no audit log against the dashboard user, no read-only role.
...
- **No protection from prompt injection in finding output displayed to LLMs.** If you paste a finding's "agent-ready follow-up" into a chat with a code-edit-capable agent, and that finding included attacker-controlled text from a scanned repo, the agent could be steered. Read the prompt before pasting.
