...
2. **The dashboard `127.0.0.1` binding.** The dashboard server binds to localhost only, not `0.0.0.0`. Sharing requires deliberate effort (SSH tunnel, explicit reverse proxy). It is not reachable from the LAN as configured. Loopback is *not* the same as private, though: any web page the operator opens in a same-machine browser can send requests to `127.0.0.1`, so the mutating API is guarded against cross-site and DNS-rebinding forgery — see risk #3 below.
...
| 3a | **Loopback-browser cross-site / DNS-rebinding forgery.** Any web page the operator opens in a same-machine browser — a malicious tab, or a site that rebinds its own DNS name to `127.0.0.1` — can issue requests to the dashboard. Without a guard, such a page could forge mutating POSTs (`/api/case-decision`, honey-insert, package install, scan reset, rotation trigger) with no click, and in particular suppress a high/critical case, defeating the human-confirmation gate. | A web page the operator visits while the dashboard is running. | Every mutating `do_POST`/`do_DELETE` is rejected with a JSON `403` unless its `Origin`/`Sec-Fetch-Site` is the dashboard's own loopback origin (the browser stamps a foreign origin a script cannot forge, which also defeats DNS rebinding because the rebound host is not loopback); mutating requests must be `Content-Type: application/json`; and a high/critical suppression additionally requires a per-session confirmation token that is only handed out on a same-origin read, so it cannot be inferred from the mere arrival of a POST. The honey-key trigger callback (`/api/honey/trigger`) is the one deliberate exemption — a honeytoken must beacon cross-origin or the decoy is useless. | The token rotates per server process and there is no allowance for a second operator; this guard assumes the single-operator, no-RBAC model. A non-browser client on the machine (curl, the CLI) is unaffected, because it was never the cross-site threat — a browser cannot omit `Origin` on a cross-origin request. |
